...
export default async function BugReportsPage({ searchParams }: { searchParams?: Record<string, string | string[]> }) {
    ...
    const statusFilterRaw = typeof searchParams?.status === 'string'
        ? searchParams?.status
        : Array.isArray(searchParams?.status)
            ? searchParams?.status[0]
            : undefined;
    const statusFilter = statusFilterRaw && ['unchecked', 'in_progress', 'resolved'].includes(statusFilterRaw)
        ? statusFilterRaw
        : 'all';
    const sortModeRaw = typeof searchParams?.sort === 'string'
        ? searchParams?.sort
    ...
    return (
        <div className="space-y-6">
            ...
            {hasStatusColumn && (
                <form className="flex flex-col gap-3 text-sm sm:flex-row sm:flex-wrap sm:items-center" method="GET">
                    <label className="text-gray-600">
                        Status
                        <select
                            name="status"
                            defaultValue={statusFilter}
                            className="mt-1 block w-full rounded-md border border-gray-300 bg-white px-3 py-2 text-sm sm:ml-2 sm:mt-0 sm:inline-block sm:w-auto"
                        >
                    ...
                    <button
                        type="submit"
                        className="touch-target rounded-md border border-blue-200 bg-blue-50 px-3 py-2 text-sm font-semibold text-blue-700 hover:bg-blue-100"
                    >
                        Apply
                    </button>
                </form>
                        ...
                        {safeReports.map((report) => (
                            <article key={report.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    {hasStatusColumn && <BugReportStatusSelect reportId={report.id} status={report.status} />}
                                </div>

                                <details className="mt-3 rounded-md border border-gray-200 bg-gray-50 px-3 py-2 text-sm text-gray-700">
                                    <summary className="cursor-pointer text-sm font-medium text-blue-600">View description</summary>
                                ...
                                <div className="mt-3 flex items-center justify-between text-sm">
                                    <span className="text-gray-500">{new Date(report.created_at).toLocaleString()}</span>
                                    {signedUrlMap[report.id] ? (
                                        <a
                                            className="touch-target inline-flex items-center rounded px-2 py-1 font-semibold text-blue-600 hover:bg-blue-50 hover:text-blue-900"
